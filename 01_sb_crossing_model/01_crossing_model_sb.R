...
area_fx <- function(i){
  ...
  x = c(102, 120, 120, unique(dist$actor.x))
  y = c(ifelse(unique(dist$actor.y>=40), 18, 62), ifelse(unique(dist$actor.y>=40), 18, 62), ifelse(unique(dist$actor.y>=40), 62, 18), unique(dist$actor.y))
  
  dist <- cbind(dist, 
        sp::point.in.polygon(dist$loc.x, dist$loc.y, x, y, mode.checked=F))
 
  names(dist)[8] <- "in_path"
  
  dist <- dist %>%
    mutate(in_path = ifelse(in_path!=0, 1, 0))
  ...
  dist_2_inpath <- ifelse(is_empty(dist_2_inpath), 0, dist_2_inpath)
  
  area %>% 
    mutate(dist_1 = dist_1,
           dist_2 = dist_2,
           dist_1_inpath = dist_1_inpath,
           dist_2_inpath = dist_2_inpath)
...
crosses <- crosses %>%
  ...
  mutate(area_delta = team_area-opp_area,
         ...
         dist_2_inpath = ifelse(dist_2_inpath==0, goal_dist, dist_2_inpath))
...
last_rf_mod <- 
  rand_forest(mtry = rf_best %>% pull(mtry), min_n = rf_best %>% pull(min_n), trees = 1000) %>% 
...
plot_input <- ffs %>%
  filter(id=="ace0d420-070d-422d-b1c4-fa1dc9211bdd") %>%
...
final_model <- pull_workflow_fit(rf_fit)

vip::vip(final_model$fit)

library(fastshap)

X <- prep(cross_rec, test_data) %>% 
  juice() %>% 
  select(-c(id, shot_assists)) %>%
  as.matrix()
...
shap <- explain(final_model$fit, X=X, nsim=5, pred_wrapper=pfun)
...
plot_fx <- function(i){
  ...
  p2 <- autoplot(shap, type = "contribution", row_num = row_num) 
  
  gridExtra::grid.arrange(p1,p2,ncol=2,widths=c(2,1))
  
}

plot_fx("7011453c-ee9c-47c4-86fd-3e1e0b970a47")
